...
def create_content_database(
    ...
    # Close the raw descriptor so the file can be removed later on Windows;
    # sqlite reopens the file by path.
    os.close(fh)
    ...
    # No matter what, make sure publishing is set to False once the run is done
        ...
        if kolibri_temp_db and os.path.exists(kolibri_temp_db):
            try:
                ...
                # open; leaving the temp file behind is preferable to failing.
                logging.warning(
                    "Could not remove temporary database file: %s", kolibri_temp_db
                )
        if not is_draft_version:
            base_tree.publishing = False
            base_tree.save()
