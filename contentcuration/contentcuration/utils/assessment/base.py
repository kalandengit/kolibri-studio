...
class ExerciseArchiveGenerator(ABC):
    ...
    def _create_zipfile(self):
        ...
        # delete=False + explicit close so the file can be reopened by name on
        # Windows, where an open NamedTemporaryFile cannot be opened twice.
        ...
        try:
            ...
            file_size = os.path.getsize(tempf.name)

            self.ccnode.files.filter(preset_id=self.preset).delete()

            with open(tempf.name, "rb") as zip_fobj:
                assessment_file_obj = models.File.objects.create(
                    file_on_disk=File(zip_fobj, name=filename),
